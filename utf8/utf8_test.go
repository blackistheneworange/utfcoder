...
func TestConvertToUTF16BE(t *testing.T) {
	for idx := 0; idx < len(utf8To16BigEndianTestInputs); idx += 2 {
		input := utf8To16BigEndianTestInputs[idx]
		expected := utf8To16BigEndianTestInputs[idx+1]
		output, err := ConvertToUTF16(input, types.UTF_16BE, false)
...
func TestConvertToUTF16BEWithBOM(t *testing.T) {
	input := utf8To16BigEndianTestInputs[0]
	expected := []byte{0xfe, 0xff}
	expected = append(expected, utf8To16BigEndianTestInputs[1]...)
	output, err := ConvertToUTF16(input, types.UTF_16BE, true)
...
func TestConvertToUTF16LEWithBOM(t *testing.T) {
	input := utf8To16LittleEndianTestInputs[0]
	expected := []byte{0xff, 0xfe}
	expected = append(expected, utf8To16LittleEndianTestInputs[1]...)
...
var utf8To16BigEndianTestInputs = [][]byte{
	{65}, {0, 65}, // 'A' (U+0041)
	{122}, {0, 122}, // 'z' (U+007A)
	{32}, {0, 32}, // space (U+0020)

	{195, 169}, {0, 233}, // é (U+00E9)
	{194, 169}, {0, 169}, // © (U+00A9)
	{194, 176}, {0, 176}, // ° (U+00B0)
	{195, 177}, {0, 241}, // ñ (U+00F1)

	{208, 144}, {4, 16}, // А (U+0410)
	{209, 129}, {4, 65}, // с (U+0441)

	{223, 191}, {7, 255}, // ߿ (U+07FF)
	{237, 159, 191}, {215, 255}, // ퟿ (U+D7FF)

	{240, 144, 128, 128}, {0xD8, 0x00, 0xDC, 0x00}, // 𐀀 (U+10000)
	{240, 144, 128, 129}, {0xD8, 0x00, 0xDC, 0x01}, // 𐀁 (U+10001)

	{240, 159, 152, 128}, {0xD8, 0x3D, 0xDE, 0x00}, // 😀 (U+1F600)

	{244, 143, 191, 191}, {0xDB, 0xFF, 0xDF, 0xFF}, // U+10FFFF

	{239, 191, 189}, {255, 253}, // U+FFFD (replacement)

	{244, 129, 128, 128},     // U+101000 (valid PUA)
	{0xDB, 0xC4, 0xDC, 0x00}, // BE surrogate pair for U+101000
	// (converted from LE {196,219,0,220})

	{225, 131, 191}, {16, 255}, // U+10FF

	{195, 191}, {0, 255}, // ÿ (U+00FF)

	{239, 191, 189}, {255, 253}, // U+FFFD replacement
	{239, 191, 189}, {255, 253}, // invalid → U+FFFD

	{224, 162, 128}, {8, 128}, // U+0880

	{226, 130, 172}, {32, 172}, // € (U+20AC)
	{226, 130, 185}, {32, 185}, // ₹ (U+20B9)

	{227, 128, 173}, {48, 45}, // ㌭ (U+302D)

	{}, {}, // empty input

	{237, 156, 128}, {0xD7, 0x00}, // U+D700 valid BMP
}
